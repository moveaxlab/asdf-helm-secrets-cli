...
setup_helm_plugin_usage() {
	rm -rf "${HELM_PLUGINS_FOLDER}/helm-secrets-cli" || true
	mkdir -p "${HELM_PLUGINS_FOLDER}" || fail "Could not create directory '${HELM_PLUGINS_FOLDER}'"
	ln -s "${ASDF_INSTALL_PATH}/bin/plugins/helm-secrets-cli" "${HELM_PLUGINS_FOLDER}/helm-secrets-cli" || fail "Could not link plugin to helm"
}
